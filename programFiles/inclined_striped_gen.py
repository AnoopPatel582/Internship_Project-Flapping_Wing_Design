...
MEM_HEIGHT      = CHORD - SPAR_HEIGHT          # membrane height
SPAR_BOTTOM     = MEM_HEIGHT                   # y where spar starts
STIFF_X_BOTTOM  = MEM_HEIGHT / np.tan(THETA)  # base of membrane triangle
STIFF_TOP_X     = STIFF_WIDTH / np.sin(THETA) # stiffener top width
...
def calc_type_b_substrip(x_s, x_e, global_x_offset=0.0):
    ...
    total_mass = total_EA = total_EAy = sum_mcgx = sum_mcgy = 0.0
    ...
    return (sum_mcgx/total_mass, sum_mcgy/total_mass,
            total_EAy/total_EA, total_mass)
...
for i in range(NUM_PAIRS):
    ...
    for k in range(NUM_SUBSTRIPS):
        x_s = k * sub_width
        x_e = x_s + sub_width
        cg_x, cg_y, na_y, mass = calc_type_b_substrip(
            x_s, x_e, global_x_offset=x_current)
        all_cg_x.append(cg_x)
        all_cg_y.append(cg_y)
        all_na_y.append(na_y)
        all_masses.append(mass)
...
for i in range(NUM_PAIRS):
    ...
    for k in range(NUM_SUBSTRIPS + 1):
        ax.axvline(x=xs + k*sub_width, color='white',
...
ax.set_title(f'Wing Layout — CG and Neutral Axis  '
             f'(SPAR_HEIGHT={SPAR_HEIGHT}mm, STIFF_WIDTH={STIFF_WIDTH}mm)',
...
plt.suptitle(f'Rectangular Wing with Inclined Stiffeners (θ={np.degrees(THETA):.0f}°)\n'
             f'CG Line vs Neutral Axis  |  '
             f'TYPE_B_WIDTH={TYPE_B_WIDTH:.3f}mm  |  WINGSPAN={WINGSPAN:.3f}mm',
